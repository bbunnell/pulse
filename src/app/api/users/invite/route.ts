...
export async function POST(request: Request) {
  ...
  if (result.status === "queued") {
    console.log(`\n[TimeBoard] Temp password for ${body.email}: ${tempPassword}\n`);
  }
...
function buildPlainText(firstName: string, email: string, password: string, loginUrl: string): string {
  return [
    `Hi ${firstName},`,
    ``,
    `Your TimeBoard account has been created. Here's everything you need to get started.`,
    ``,
    `─── STEP 1: Sign in to TimeBoard ───────────────────────────────`,
    ``,
    `  URL:      ${loginUrl}`,
    `  Email:    ${email}`,
    `  Password: ${password}`,
    ...
    ``,
    `TimeBoard can send you a direct Teams message when it's time to`,
    ...
    `     Give it a name like "TimeBoard Reminder".`,
    ``,
    ...
    `     to your TimeBoard profile. Once added, reminders will`,
    `     be sent directly to you in Teams.`,
    ``,
    `─────────────────────────────────────────────────────────────────`,
    ``,
    `Questions? Reply to this email or contact your TimeBoard admin.`,
    ``,
    `— TimeBoard`,
  ].join("\n");
}
  ...
  <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.08);">
    ...
    <tr><td style="padding:28px 32px 0;">
      ...
      <table cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px 20px;width:100%;">
        <tr>
          ...
          <td style="font-size:13px;color:#0F172A;"><a href="${loginUrl}" style="color:#4F46E5;">${loginUrl}</a></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            <span style="font-family:monospace;font-size:16px;font-weight:700;letter-spacing:2px;background:#EEF2FF;color:#3730A3;padding:4px 10px;border-radius:5px;">${password}</span>
          </td>
        </tr>
      </table>
    ...
    <tr><td style="padding:24px 32px 0;">
      ...
      <p style="margin:0 0 16px;font-size:14px;color:#334155;line-height:1.6;">
        TimeBoard can send you a <strong>direct Teams message</strong> when it's time to clock in or clock out.
        This takes about 2 minutes to set up using Power Automate.
      </p>

      <!-- Steps -->
      ${[
        ["Go to <a href='https://make.powerautomate.com' style='color:#4F46E5;'>make.powerautomate.com</a> and sign in with your NBIT Microsoft account.", null],
        ["Click <strong>New flow</strong> → <strong>Instant cloud flow</strong>. Name it something like <em>\"TimeBoard Reminder\"</em>.", null],
        ...
        <table cellpadding="0" cellspacing="0" style="margin-bottom:12px;width:100%;">
          <tr>
            <td style="width:28px;vertical-align:top;padding-top:1px;">
              <span style="display:inline-block;width:20px;height:20px;background:#EEF2FF;color:#4F46E5;border-radius:50%;font-size:11px;font-weight:700;text-align:center;line-height:20px;">${i + 1}</span>
            </td>
    ...
    <tr><td style="padding:24px 32px 28px;">
      ...
      <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.5;">
        Questions? Contact your TimeBoard administrator.<br>
        This is an automated message from TimeBoard — please do not reply directly to this email.
      </p>
    </td></tr>
